...
from lib.entities.entity import (
  Entity, PersonEntity, CustomAttributeDefinitionEntity, ProgramEntity, ControlEntity,
  ObjectiveEntity, AuditEntity, AssessmentTemplateEntity, AssessmentEntity,
  IssueEntity, CommentEntity, Representation)
...
class EntitiesFactory(object):
  ...
  def __init__(self, obj_name, obj_creator):
    ...
    self.obj_type = self.get_obj_type(self.obj_name)
    ...
    if self.child_cls not in [
        AssessmentTemplatesFactory, CustomAttributeDefinitionEntity,
        PersonEntity, CommentEntity]:
      self.obj_slug = self.generate_slug()

  @staticmethod
  def get_obj_type(obj_name):
    """Get object's type based on object's name."""
    return unicode(objects.get_singular(obj_name, title=True))
  ...
  def create(self, is_add_rest_attrs=True, **attrs):
...
class PeopleFactory(EntitiesFactory):
  ...
  def __init__(self, obj_name=objects.PEOPLE):
    ...
    if self.child_cls not in [AssessmentTemplatesFactory,
      ...
      if self.child_cls == ProgramsFactory:
        ...
        self.admins_names = self.extract_people_names(
            help_utils.convert_to_list(self.admins))
      if self.child_cls == AssessmentsFactory:
        self.assignees = [self.default_user]
        self.assignees_names = self.extract_people_names(
            help_utils.convert_to_list(self.assignees))
        self.verifiers = [self.default_user]
        self.verifiers_names = self.extract_people_names(
            help_utils.convert_to_list(self.verifiers))
      if self.child_cls == ControlsFactory:
        self.principal_assignees = [self.default_user]
        self.principal_assignees_names = self.extract_people_names(
            help_utils.convert_to_list(self.principal_assignees))
        self.secondary_assignees = [self.default_user]
        self.secondary_assignees_names = self.extract_people_names(
            help_utils.convert_to_list(self.secondary_assignees))
      if self.child_cls != AuditsFactory:
        self.primary_contacts = [self.default_user]
        self.primary_contacts_names = self.extract_people_names(
            help_utils.convert_to_list(self.primary_contacts))
        self.secondary_contacts = [self.default_user]
        self.secondary_contacts_names = self.extract_people_names(
            help_utils.convert_to_list(self.secondary_contacts))
      else:
        self.auditors = [self.default_user]
        self.auditors_names = self.extract_people_names(
            help_utils.convert_to_list(self.auditors))

  @staticmethod
  def extract_people_names(people):
    """Extract values for person's name attributes."""
    return [
        person.name for person in help_utils.convert_to_list(people)
  ...
  def get_acl_member(role_id, person):
    ...
    if isinstance(person, PersonEntity):
      ...
      raise ValueError(messages.CommonMessages.err_common(
          PersonEntity, person))
...
class ProgramsFactory(PeopleFactory):
  ...
  primary_contacts_acl_id = 33
  secondary_contacts_acl_id = 34
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    issue_obj = self.obj_inst().update_attrs(
      ...
      manager=self.manager_name, primary_contacts=self.primary_contacts_names,
      secondary_contacts=self.secondary_contacts_names,
    ...
    if is_add_rest_attrs:
      issue_obj.update_attrs(
        recipients=",".join((
            ...
            unicode(roles.SECONDARY_CONTACTS))),
        access_control_list=string_utils.StringMethods.convert_list_elements_to_list(
            [self.get_acl_members(self.primary_contacts_acl_id,
                                  self.primary_contacts),
             self.get_acl_members(self.secondary_contacts_acl_id,
...
class ControlsFactory(PeopleFactory):
  ...
  admins_acl_id = 49
  principal_assignees_acl_id = 11
  secondary_assignees_acl_id = 12
  primary_contacts_acl_id = 9
  secondary_contacts_acl_id = 10
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    control_obj = self.obj_inst().update_attrs(
        title=self.obj_title, slug=self.obj_slug,
        status=unicode(element.ObjectStates.DRAFT),
        admins=self.admins_names,
        principal_assignees=self.principal_assignees_names,
        primary_contacts=self.primary_contacts_names,
        secondary_contacts=self.secondary_contacts_names,
        os_state=unicode(element.ReviewStates.UNREVIEWED))
    if is_add_rest_attrs:
      control_obj.update_attrs(
          ...
          access_control_list=string_utils.StringMethods.convert_list_elements_to_list(
              [self.get_acl_members(self.admins_acl_id, self.admins),
               self.get_acl_members(self.principal_assignees_acl_id,
                                    self.principal_assignees),
               self.get_acl_members(self.primary_contacts_acl_id,
                                    self.primary_contacts),
               self.get_acl_members(self.secondary_contacts_acl_id,
                                    self.secondary_contacts)]))
...
class ObjectivesFactory(PeopleFactory):
  ...
  admins_acl_id = 55
  primary_contacts_acl_id = 21
  secondary_contacts_acl_id = 22
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    objective_obj = self.obj_inst().update_attrs(
        title=self.obj_title, slug=self.obj_slug,
        status=unicode(element.ObjectStates.DRAFT),
        admins=self.admins_names, primary_contacts=self.primary_contacts_names,
        secondary_contacts=self.secondary_contacts_names,
        os_state=unicode(element.ReviewStates.UNREVIEWED))
    if is_add_rest_attrs:
...
class AuditsFactory(PeopleFactory):
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    asmt_obj = self.obj_inst().update_attrs(
      title=self.obj_title, slug=self.obj_slug,
      audit_captains=self.admins_names,
...
class AssessmentTemplatesFactory(PeopleFactory):
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    asmt_tmpl_obj = self.obj_inst().update_attrs(
        ...
        status=unicode(element.ObjectStates.DRAFT),
        template_object_type=ControlsFactory().obj_type)
    if is_add_rest_attrs:
...
class AssessmentsFactory(PeopleFactory):
  ...
  assignees_acl_id = 72
  verifiers_acl_id = 73
  primary_contacts_acl_id = 3
  secondary_contacts_acl_id = 4
  ...
  def generate(self, mapped_objects, audit, asmt_tmpl=None):
    ...
    """
    asmts_creators = self.admins_names
  ...
  def create(self, is_add_rest_attrs=True, **attrs):
    ...
    if attrs.get("verifiers"):
      asmt_obj.update_attrs(
          access_control_list=string_utils.StringMethods.convert_list_elements_to_list(
            [asmt_obj.access_control_list,
             self.get_acl_members(self.verifiers_acl_id, self.verifiers)]))
    return asmt_obj
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    asmt_obj = self.obj_inst().update_attrs(
        title=self.obj_title, slug=self.obj_slug,
        status=unicode(element.AssessmentStates.NOT_STARTED),
        creators=self.admins_names, assignees=self.assignees_names,
        primary_contacts=self.primary_contacts_names,
        secondary_contacts=self.secondary_contacts_names,
        assessment_type=self.get_obj_type(objects.CONTROLS),
        os_state=unicode(element.ReviewStates.UNREVIEWED), verified=False)
    if is_add_rest_attrs:
      asmt_obj.update_attrs(
          recipients=",".join((
...
class IssuesFactory(PeopleFactory):
  ...
  admins_acl_id = 53
  primary_contacts_acl_id = 17
  secondary_contacts_acl_id = 18
  ...
  def _create_random_obj(self, is_add_rest_attrs):
    ...
    return issue_obj
#
# h = CustomAttributesFactory().create()
#
# j = h
