...
class CommonProgram(Common):
  ...
  PROGRAM = objects.Utils.get_normal_form(
      objects.Utils.get_singular(objects.Names.PROGRAMS))
  TITLE = Common.TITLE
  MANAGER = "Manager"
  NOTES = "Notes"
...
class CommonAudit(Common):
  ...
  # pylint: disable=too-many-instance-attributes
  AUDIT = objects.Utils.get_normal_form(objects.Utils.get_singular(objects.Names.AUDITS))
  STATUS = "Status"
...
class CommonControl(Common):
  ...
  CONTROL = objects.Utils.get_normal_form(
      objects.Utils.get_singular(objects.Names.CONTROLS))
  STATE = Base.STATE
  ADMIN = roles.GlobalRoles.ADMIN
  PRIMARY_CONTACTS = Common.PRIMARY_CONTACTS
...
class CommonObjective(Common):
  ...
  OBJECTIVE = objects.Utils.get_normal_form(
      objects.Utils.get_singular(objects.Names.OBJECTIVES))
  STATE = Base.STATE
  ADMIN = roles.GlobalRoles.ADMIN
  PRIMARY_CONTACTS = Common.PRIMARY_CONTACTS
  MAPPED_OBJECTS = TransformationSetVisibleFields.MAPPED_OBJECTS
...
class CommonAssessment(Common):
  ...
  ASMT = objects.Utils.get_normal_form(
      objects.Utils.get_singular(objects.Names.ASSESSMENTS))
  STATE = Base.STATE
  CREATORS = roles.AssessmentRoles.CREATORS
  COMMENTS = "Comments"
  ASSIGNEES = roles.AssessmentRoles.ASSIGNEES
...
class CommonAssessmentTemplate(Common):
 ...
 """
  ASMT_TMPL = objects.Utils.get_normal_form(
      objects.Utils.get_singular(objects.Names.ASSESSMENT_TEMPLATES))


class CommonIssue(Common):
  """Common elements' labels and properties for Issues objects."""
  ISSUE = objects.Utils.get_normal_form(objects.Utils.get_singular(objects.Names.ISSUES))
  STATE = Base.STATE
